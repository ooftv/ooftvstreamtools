...
# -n option skips files that have already been compressed ; which on 2 pass skips everything since /dev/null already exists
# try something like:
#for file in *.flv
#do
#  mp4=${file%%.*}.mp4
...
compress() {
   ...
   #ls #debugging
   for f in $1/*.mp4; #finds all files in the folder that have a fileextension starting with m (mp4, m4v, mov)
   do
...
if [[ ! -e "$1/compressed/"$name"-broadcast.mp4" ]]; then
       ...
       /usr/local/bin/ffmpeg -y -i "$f" -vf scale=1280:720 -c:v libx264 -b:v 1500k -pass 1 -c:a aac -b:a 128k -f mp4 /dev/null && /usr/local/bin/ffmpeg -    i "$f" -vf scale=1280:720 -c:v libx264 -b:v 1500k -pass 2 -c:a aac -b:a 128k "compressed/${f%.*}-broadcast.mp4"
   fi
   done
}
